...
function WaFloatingButton() {
  ...
  useEffect(() => {
    if (!open) return;
    const handler = (e) => {
      if (!e.target.closest("#wa-float-wrap")) setOpen(false);
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, [open]);

  const color = waReady ? "#25d366" : "#94a3b8";
  const shadow = waReady ? "0 4px 20px rgba(37,211,102,0.45)" : "0 4px 16px rgba(0,0,0,0.2)";

  const menuBtn = (label, sub, onClick) => (
    <button
      onClick={onClick}
      style={{
        display: "flex", flexDirection: "column", alignItems: "flex-start",
        gap: 2, width: "100%", background: "transparent", border: "none",
    ...
    >
      <span style={{ fontSize: 13, fontWeight: 600, color: "#e9edef" }}>{label}</span>
      <span style={{ fontSize: 11, color: "#8696a0", lineHeight: 1.4 }}>{sub}</span>
    </button>
  );

  return (
    <div id="wa-float-wrap" style={{ position: "fixed", bottom: 28, right: 28, zIndex: 1000 }}>
      {/* Popup menu */}
      {open && (
        <div style={{
          ...
          <div style={{ padding: "8px 12px 6px", borderBottom: "1px solid rgba(255,255,255,0.07)", marginBottom: 2 }}>
            <span style={{ fontSize: 11, color: "#667781", fontWeight: 600, letterSpacing: "0.5px", textTransform: "uppercase" }}>
              Buka WhatsApp Web
            </span>
          </div>
          ...
          {menuBtn(
            ...
            "Real WA Web — cepat & responsif. Pesan sama, perlu scan QR sekali.",
            () => {
          ...
          {menuBtn(
            "🤖 Sesi Bot (Sama Persis)",
            "Lihat & kontrol sesi yang dipakai bot. Ada sedikit delay.",
            () => {
              window.open("/wa-web", "_blank", "noopener,noreferrer");
              setOpen(false);
            }
          )}
        </div>
