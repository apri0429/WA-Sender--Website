...
export default function WaWebFullscreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(sendSize, 150);
    window.addEventListener("resize", sendSize);
    return () => {
      clearTimeout(timer);
      window.removeEventListener("resize", sendSize);
      // Reset viewport ke default saat tab ditutup
  ...
  const flush = () => {
    if (keyBuf.current) { socket.emit("wa-type", { text: keyBuf.current }); keyBuf.current = ""; }
  ...
  const handleKeyDown = (e) => {
    ...
    if (specials.includes(e.key)) {
      ...
      keyTimer.current = setTimeout(flush, 30);
    }
  };

  const dotColor = waReady ? "#22c55e" : connected ? "#f59e0b" : "#94a3b8";
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", display: "flex", flexDirection: "column", background: "#000", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        padding: "5px 14px", background: "#075e54", flexShrink: 0,
      }}>
        ...
        <span style={{ width: 8, height: 8, borderRadius: "50%", background: dotColor, boxShadow: waReady ? `0 0 6px ${dotColor}` : "none", flexShrink: 0 }} />
        ...
        {!connected && (
          <div style={{
            ...
            <div style={{ fontSize: 13, color: "#9ca3af", textAlign: "center", maxWidth: 300 }}>
              {waReady ? "Harap tunggu..." : "Login dulu di halaman Dashboard, lalu buka kembali halaman ini."}
            </div>
          </div>
        )}
        <img
